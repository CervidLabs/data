...
export class Octopus {
    ...
    static async _readCSV(filePath, options = {}) {
        ...
        let offsetBuffer = useOffsets ? new SharedArrayBuffer(capacity * totalCols * 2 * 4) : null;
        ...
        for (let i = 0; i < numWorkers; i++) {
            ...
            promises.push(new Promise((resolve) => {
                const worker = new Worker("./src/workers/ingest.worker.js", {
                    workerData: {
                        sharedBuffer, offsetBuffer, colBuffers, start, end,
...
// In _readJSON logic:
        const rows = data.flatMap(item => {
            const nestedArrayKey = Object.keys(item).find(k => Array.isArray(item[k]));
            if (nestedArrayKey) {
